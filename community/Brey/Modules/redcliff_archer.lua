...
-- Place in: UhhhhhhReanim/Modules/redcliff_archer.lua
...
AddModule(function()
    ...
    m.Name         = "redcliff archer"
    m.Description  = "\"the archers of the redcliff empire. A glasscannon to korbloxes perhaps.\""
    m.InternalName = "redcliff_archer"

    m.Assets = {
        "archeridle.anim",
        "archerwalk.anim",
        "Archerprojectile.anim",
        "archercharge.anim",
        "archerchargeshot.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "redcliff archer", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "archeridle.anim", true),
        walk = loadAnim(figure, "archerwalk.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["fire"] = loadAnim(figure, "Archerprojectile.anim", false),
        ["charge"] = loadAnim(figure, "archercharge.anim", false),
        ["super shot"] = loadAnim(figure, "archerchargeshot.anim", false),
        }
        ...
        ContextActions:BindAction("GM_fire", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("fire") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_fire", "fire")
        ContextActions:SetPosition("GM_fire", UDim2.new(1, -290, 1, -130))

        ContextActions:BindAction("GM_charge", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("charge") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_charge", "charge")
        ContextActions:SetPosition("GM_charge", UDim2.new(1, -180, 1, -130))

        ContextActions:BindAction("GM_super_shot", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("super shot") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_super_shot", "super shot")
        ContextActions:SetPosition("GM_super_shot", UDim2.new(1, -230, 1, -130))
    end
    ...
    m.Destroy = function(figure)
        ContextActions:UnbindAction("GM_fire")
        ContextActions:UnbindAction("GM_charge")
        ContextActions:UnbindAction("GM_super_shot")
