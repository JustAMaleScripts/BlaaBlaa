...
-- Place in: UhhhhhhReanim/Modules/bubonic_plant.lua
...
AddModule(function()
    ...
    m.Name         = "bubonic plant"
    m.Description  = "\"yo can you not be suicidal just to become a plant\""
    m.InternalName = "bubonic_plant"
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "bubonic plant", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "bubonicidle.anim", true),
        walk = loadAnim(figure, "bubonicwalk.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["bite"] = loadAnim(figure, "bubonicbigbite.anim", false),
        ["tendril"] = loadAnim(figure, "bubonictendril.anim", false),
        ["Plant rush"] = loadAnim(figure, "bubonicplantrush.anim", false),
        }
        ...
        ContextActions:BindAction("GM_bite", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("bite") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_bite", "bite")
        ContextActions:SetPosition("GM_bite", UDim2.new(1, -230, 1, -130))

        ContextActions:BindAction("GM_tendril", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("tendril") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_tendril", "tendril")
        ContextActions:SetPosition("GM_tendril", UDim2.new(1, -230, 1, -190))

        ContextActions:BindAction("GM_Plant_rush", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Plant rush") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Plant_rush", "Plant rush")
        ContextActions:SetPosition("GM_Plant_rush", UDim2.new(1, -190, 1, -150))
    end
    ...
    m.Destroy = function(figure)
        ContextActions:UnbindAction("GM_bite")
        ContextActions:UnbindAction("GM_tendril")
        ContextActions:UnbindAction("GM_Plant_rush")
