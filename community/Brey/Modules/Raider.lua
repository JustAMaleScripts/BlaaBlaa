...
-- Place in: UhhhhhhReanim/Modules/Raider.lua
...
AddModule(function()
    ...
    m.Name         = "Raider"
    m.Description  = "\"only I get to loot these items in the temple you freak.\""
    m.InternalName = "Raider"

    m.Assets = {
        "raideridle.anim",
        "raiderwalk.anim",
        "raiderslashkick.anim",
        "raiderflipkick.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Raider", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "raideridle.anim", true),
        walk = loadAnim(figure, "raiderwalk.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["slash kick"] = loadAnim(figure, "raiderslashkick.anim", false),
        ["flipkick"] = loadAnim(figure, "raiderflipkick.anim", false),
        }
        ...
        ContextActions:BindAction("GM_slash_kick", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("slash kick") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_slash_kick", "slash kick")
        ContextActions:SetPosition("GM_slash_kick", UDim2.new(1, -230, 1, -130))

        ContextActions:BindAction("GM_flipkick", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("flipkick") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_flipkick", "flipkick")
        ContextActions:SetPosition("GM_flipkick", UDim2.new(1, -290, 1, -130))
    end
    ...
    m.Destroy = function(figure)
        ContextActions:UnbindAction("GM_slash_kick")
        ContextActions:UnbindAction("GM_flipkick")
