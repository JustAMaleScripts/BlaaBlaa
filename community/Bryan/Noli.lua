...
m.Assets = {
    ...
    "noli ab1 - dash startup.anim",
    "noli ab1 - dash loop.anim",
    "noli ab1 -  dash attack.anim",

    -- VOID RUSH (ab1) — arm loop (idle hold)
    "noli ab1 - armloop.anim",

    -- VOID RUSH (ab1) — second dash chain
    "noli ab1 - second dash startup.anim",
    "noli ab1 - second dash loop.anim",
    "noli ab1 - second dash hit.anim",

    -- NOVA (ab2) — cast & recast
    "noli nova - throw [reanimation].anim",
    "noli nova - recast.anim",

    -- OBSERVANT (ab3)
    "noli observantstartup.anim",
    "noli observantloop.anim",
    "Noli teleport.anim",
...
local function onVoidRushPress()
    ...
    if voidRushPhase == "wait_second" then
        ...
        freezeCharacter()
        -- Stop arm loop if it was playing
...
m.Config = function(parent)
    Util_CreateText(parent, "Noli", 18, Enum.TextXAlignment.Center)
...
m.Init = function(figure)
    ...
    -- BASE ANIMS (looped)
    baseAnims = {
        idle   = loadAnim(figure, "Noli idle.anim", true),
        walk   = loadAnim(figure, "Noli walk.anim", true),
        sprint = loadAnim(figure, "Noli run.anim",  true),
    }

    -- ACTION ANIMS
    actionAnims = {
        -- VOID RUSH — first dash chain
        ["ab1firstdashstartup"]      = loadAnim(figure, "noli ab1 - dash startup.anim",        false),
        ["ab1firstdashloop"]         = loadAnim(figure, "noli ab1 - dash loop.anim",            true),
        ["ab1firstdashsuccesfulhit"] = loadAnim(figure, "noli ab1 -  dash attack.anim",         false),
        -- VOID RUSH — arm loop (idle hold while charging)
        ["ab1armloop"]               = loadAnim(figure, "noli ab1 - armloop.anim",              true),
        -- VOID RUSH — second dash chain
        ["ab1seconddashstartup"]     = loadAnim(figure, "noli ab1 - second dash startup.anim",  false),
        ["ab1seconddashloop"]        = loadAnim(figure, "noli ab1 - second dash loop.anim",     true),
        ["ab1succesfulhit"]          = loadAnim(figure, "noli ab1 - second dash hit.anim",      false),
        -- NOVA
        ["Nova"]                     = loadAnim(figure, "noli nova - throw [reanimation].anim",  false),
        ["nova - recast "]           = loadAnim(figure, "noli nova - recast.anim",               false),
        -- OBSERVANT
        ["observant activation"]     = loadAnim(figure, "noli observantstartup.anim",            false),
        ["observantstartup"]         = loadAnim(figure, "noli observantloop.anim",               false),
        ["teleport"]                 = loadAnim(figure, "Noli teleport.anim",                    false),
        -- M1 / KILL
        ["M1"]                       = loadAnim(figure, "Noli m1.anim",                          false),
        ["kill"]                     = loadAnim(figure, "noli_kill.anim",               false),
    }
    ...
    bgSound = makeSound("noli chase theme.mp3", hrp, 0.5, true)
    bgSound:Play()
        ...
        if a and actionStart[a] and not a.looped then
            ...
            if duration > 0 and elapsed >= duration / (a.speed or 1) then
                ...
                elseif voidRushPhase == "first_hit" then
                    -- First dash done → open second dash window, play arm loop
                    voidRushPhase = "wait_second"
                    stopAction()
                    unfreezeCharacter()
                    currentBase = "none"
                    -- Play arm loop while waiting for second dash input
