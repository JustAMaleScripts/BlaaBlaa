-- MarkGraysonInvincibleMoveset.lua
-- Created with Moveset Creator V6
-- Place in: UhhhhhhReanim/Modules/MarkGraysonInvincibleMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "Mark Grayson"
    m.Description  = "THAT TITTLE CARD, Mark GraySON"
    m.InternalName = "MarkGraysonInvincibleMoveset"

    m.Assets = {
        "Invincible Idle.anim",
        "Invincible Walk.anim",
        "Invincible Run.anim",
        "Invincible Barrage.anim",
        "Invincible FlightAttack.anim",
        "Invincible Punch.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Mark Grayson", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "Invincible Idle.anim", true),
        walk = loadAnim(figure, "Invincible Walk.anim", true),
        sprint = loadAnim(figure, "Invincible Run.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Barrage"] = loadAnim(figure, "Invincible Barrage.anim", false),
        ["Flight Attack"] = loadAnim(figure, "Invincible FlightAttack.anim", false),
        ["Punch"] = loadAnim(figure, "Invincible Punch.anim", false),
        }
        ...
        ContextActions:BindAction("GM_Barrage", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Barrage") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Barrage", "Barrage")
        ContextActions:SetPosition("GM_Barrage", UDim2.new(1, -200, 1, -110))

        ContextActions:BindAction("GM_Flight_Attack", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Flight Attack") end
        end, true, Enum.KeyCode.X)
        ContextActions:SetTitle("GM_Flight_Attack", "Flight Attack")
        ContextActions:SetPosition("GM_Flight_Attack", UDim2.new(1, -160, 1, -110))
        ...
        ContextActions:SetPosition("GM_Punch", UDim2.new(1, -240, 1, -110))
    end
    ...
    m.Destroy = function(figure)
        ...
        ContextActions:UnbindAction("GM_Barrage")
        ContextActions:UnbindAction("GM_Flight_Attack")
