...
-- Place in: UhhhhhhReanim/Modules/BIASTMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "BIAST Moveset"
    m.Description  = "Normal Player Animations from Break In And Steal Thingz"
    m.InternalName = "BIASTMoveset"

    m.Assets = {
        "BiastIdle.anim",
        "BiastWalk.anim",
        "BiastRun.anim",
        "CrouchWalkBiast.anim",
        "CrouchIdleBiast.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "BIAST Moveset", 18, Enum.TextXAlignment.Center)
    ...
    local function endCrouch()
        ...
        if figureRef then
            local hum = figureRef:FindFirstChildOfClass("Humanoid")
            if hum then hum.WalkSpeed = isSprinting and SPEED_SPRINT or SPEED_WALK end
        end
        if animCrouchIdle then animCrouchIdle.weight = 0 end
        if animCrouchWalk then animCrouchWalk.weight = 0 end
    ...
    local function toggleSprint()
        ...
        if isSprinting then
            setSpeed(isCrouching and 12 or  SPEED_SPRINT)
            ContextActions:SetTitle("GM_Sprint", "Walk")
        else
            setSpeed(isCrouching and 9 or  SPEED_WALK)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "BiastIdle.anim", true),
        walk = loadAnim(figure, "BiastWalk.anim", true),
        sprint = loadAnim(figure, "BiastRun.anim", true),
        }
        animJump = baseAnims.jump

        -- crouch anims
        animCrouchIdle = loadAnim(figure, "CrouchIdleBiast.anim", true)
        animCrouchWalk = loadAnim(figure, "CrouchWalkBiast.anim", true)
        ...
        ContextActions:SetTitle("GM_Sprint", "Sprint")
        ContextActions:SetPosition("GM_Sprint", UDim2.new(10, -80, 10, -130))
        ...
        ContextActions:SetTitle("GM_Crouch", "Crouch")
        ContextActions:SetPosition("GM_Crouch", UDim2.new(10, -130, 10, -101))
