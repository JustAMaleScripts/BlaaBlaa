...
-- Place in: UhhhhhhReanim/Modules/HandR15Moveset.lua
...
AddModule(function()
    ...
    m.Name         = "Hand Moveset"
    m.Description  = "You are a floating Hand"
    m.InternalName = "HandR15Moveset"
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Hand Moveset", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "HandIdle.anim", true),
        walk = loadAnim(figure, "HandWalk.anim", true),
        sprint = loadAnim(figure, "HandRun.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Wave"] = loadAnim(figure, "HandWave.anim", false),
        ["Point"] = loadAnim(figure, "HandPoint.anim", false),
        ["Thumbs Up"] = loadAnim(figure, "HandThumbsUp.anim", false),
        ["Thumbs Down"] = loadAnim(figure, "HandThumbsDown.anim", false),
        }
        ...
        ContextActions:SetTitle("GM_Sprint", "Sprint")
        ContextActions:SetPosition("GM_Sprint", UDim2.new(1, -140, 1, -100))


        ContextActions:BindAction("GM_Wave", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Wave") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Wave", "Wave")
        ContextActions:SetPosition("GM_Wave", UDim2.new(1, -180, 1, -100))

        ContextActions:BindAction("GM_Point", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Point") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Point", "Point")
        ContextActions:SetPosition("GM_Point", UDim2.new(1, -220, 1, -100))

        ContextActions:BindAction("GM_Thumbs_Up", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Thumbs Up") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Thumbs_Up", "Thumbs Up")
        ContextActions:SetPosition("GM_Thumbs_Up", UDim2.new(1, -180, 1, -140))

        ContextActions:BindAction("GM_Thumbs_Down", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Thumbs Down") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Thumbs_Down", "Thumbs Down")
        ContextActions:SetPosition("GM_Thumbs_Down", UDim2.new(1, -220, 1, -140))
    end
    ...
    m.Destroy = function(figure)
        ...
        ContextActions:UnbindAction("GM_Wave")
        ContextActions:UnbindAction("GM_Point")
        ContextActions:UnbindAction("GM_Thumbs_Up")
        ContextActions:UnbindAction("GM_Thumbs_Down")
