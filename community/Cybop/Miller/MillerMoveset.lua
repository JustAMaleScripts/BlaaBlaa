...
-- Place in: UhhhhhhReanim/Modules/MillerMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "Miller"
    m.Description  = "He is Just a Loud Yeller."
    m.InternalName = "MillerMoveset"
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Miller", 18, Enum.TextXAlignment.Center)
    ...
    local function stopAllAnimSounds()
        if snd_Yell then snd_Yell:Stop() end
    end
    ...
    local function playAction(name)
        ...
        -- per-action audio
        if name == "Yell" and snd_Yell then snd_Yell:Play() end
    end
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "MillerIdle.anim", true),
        walk = loadAnim(figure, "MillerWalk.anim", true),
        sprint = loadAnim(figure, "MillerRun.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Punch"] = loadAnim(figure, "MillerPunch.anim", false),
        ["Grab"] = loadAnim(figure, "MillerGrab.anim", false),
        ["Yell"] = loadAnim(figure, "MillerYell.anim", false),
        }

        -- init per-anim sounds
    snd_Yell = makeSound("MillerYellSound.mp3", hrp, 0.9, false)
        ...
        ContextActions:SetPosition("GM_Grab", UDim2.new(1, -160, 1, -110))

        ContextActions:BindAction("GM_Yell", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Yell") end
        end, true, Enum.KeyCode.C)
        ContextActions:SetTitle("GM_Yell", "Yell")
        ContextActions:SetPosition("GM_Yell", UDim2.new(1, -240, 1, -110))
    end
    ...
    m.Destroy = function(figure)
        ...
        ContextActions:UnbindAction("GM_Yell")

        for _, conn in ipairs(touchConns) do conn:Disconnect() end
        table.clear(touchConns)

        stopAllAnimSounds()
    if snd_Yell then snd_Yell:Stop() snd_Yell:Destroy() snd_Yell = nil end
