-- PlayerDummysakenMoveset.lua
-- Created with Moveset Creator V6
-- Place in: UhhhhhhReanim/Modules/PlayerDummysakenMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "Player Moveset"
    m.Description  = "Just a Player who woke up one day to find themselves in a reanimator"
    m.InternalName = "PlayerDummysakenMoveset"

    m.Assets = {
        "PlayerIdle.anim",
        "PlayerWalk.anim",
        "PlayerSprintRun.anim",
        "PlayerSwordSwing.anim",
        "PlayerIceDagger.anim",
        "PlayerGhostPotion.anim",
        "PlayerBlock.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Player Moveset", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "PlayerIdle.anim", true),
        walk = loadAnim(figure, "PlayerWalk.anim", true),
        sprint = loadAnim(figure, "PlayerSprintRun.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Sword Swing"] = loadAnim(figure, "PlayerSwordSwing.anim", false),
        ["Ice Dagger"] = loadAnim(figure, "PlayerIceDagger.anim", false),
        ["Ghost Potion"] = loadAnim(figure, "PlayerGhostPotion.anim", false),
        ["Block"] = loadAnim(figure, "PlayerBlock.anim", false),
        }
        ...
        ContextActions:BindAction("GM_Sword_Swing", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Sword Swing") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Sword_Swing", "Sword Swing")
        ContextActions:SetPosition("GM_Sword_Swing", UDim2.new(1, -200, 1, -110))

        ContextActions:BindAction("GM_Ice_Dagger", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Ice Dagger") end
        end, true, Enum.KeyCode.X)
        ContextActions:SetTitle("GM_Ice_Dagger", "Ice Dagger")
        ContextActions:SetPosition("GM_Ice_Dagger", UDim2.new(1, -160, 1, -110))

        ContextActions:BindAction("GM_Ghost_Potion", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Ghost Potion") end
        end, true, Enum.KeyCode.C)
        ContextActions:SetTitle("GM_Ghost_Potion", "Ghost Potion")
        ContextActions:SetPosition("GM_Ghost_Potion", UDim2.new(1, -240, 1, -110))
        ...
        ContextActions:SetPosition("GM_Block", UDim2.new(1, -280, 1, -110))
    end
    ...
    m.Destroy = function(figure)
        ...
        ContextActions:UnbindAction("GM_Sword_Swing")
        ContextActions:UnbindAction("GM_Ice_Dagger")
        ContextActions:UnbindAction("GM_Ghost_Potion")
