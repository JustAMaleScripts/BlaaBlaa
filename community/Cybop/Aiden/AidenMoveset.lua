...
-- Place in: UhhhhhhReanim/Modules/AidenMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "Aiden Moveset"
    m.Description  = "Where Did They Go..?"
    m.InternalName = "AidenMoveset"
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Aiden Moveset", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "Aiden Idle.anim", true),
        walk = loadAnim(figure, "Aiden Walk.anim", true),
        sprint = loadAnim(figure, "Aiden Run.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Axe"] = loadAnim(figure, "Aiden AxeSwing.anim", false),
        ["SledgeHammer"] = loadAnim(figure, "Aiden SledgeHammer.anim", false),
        ["Shotgun"] = loadAnim(figure, "Aiden Shotgun.anim", false),
        ["Sniper"] = loadAnim(figure, "Aiden Sniper.anim", false),
        }
        ...
        ContextActions:BindAction("GM_Axe", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Axe") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Axe", "Axe")
        ContextActions:SetPosition("GM_Axe", UDim2.new(1, -200, 1, -110))

        ContextActions:BindAction("GM_SledgeHammer", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("SledgeHammer") end
        end, true, Enum.KeyCode.X)
        ContextActions:SetTitle("GM_SledgeHammer", "SledgeHammer")
        ContextActions:SetPosition("GM_SledgeHammer", UDim2.new(1, -160, 1, -110))
        ...
        ContextActions:SetPosition("GM_Shotgun", UDim2.new(1, -200, 1, -150))

        ContextActions:BindAction("GM_Sniper", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Sniper") end
        end, true, Enum.KeyCode.V)
        ContextActions:SetTitle("GM_Sniper", "Sniper")
        ContextActions:SetPosition("GM_Sniper", UDim2.new(1, -160, 1, -150))
    end
    ...
    m.Destroy = function(figure)
        ...
        ContextActions:UnbindAction("GM_Axe")
        ContextActions:UnbindAction("GM_SledgeHammer")
        ContextActions:UnbindAction("GM_Shotgun")
        ContextActions:UnbindAction("GM_Sniper")
