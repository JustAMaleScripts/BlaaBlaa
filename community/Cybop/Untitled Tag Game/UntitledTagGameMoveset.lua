-- UntitledTagGameMoveset.lua
-- Created with Moveset Creator V6
-- Place in: UhhhhhhReanim/Modules/UntitledTagGameMoveset.lua
...
AddModule(function()
    ...
    m.Name         = "Untitled Tag Game Moveset"
    m.Description  = "This Moveset is based in the animations player of the game called Untitled Tag Game"
    m.InternalName = "UntitledTagGameMoveset"

    m.Assets = {
        "UntitledTagGameIdle.anim",
        "UntitledTagGameRun.anim",
        "UntitledTagGameJump.anim",
        "UntitledTagGameTag1.anim",
        "UntitledTagGameTag2.anim",
    }
    ...
    m.Config = function(parent)
        Util_CreateText(parent, "Untitled Tag Game Moveset", 18, Enum.TextXAlignment.Center)
    ...
    m.Init = function(figure)
        ...
        baseAnims = {
        idle = loadAnim(figure, "UntitledTagGameIdle.anim", true),
        walk = loadAnim(figure, "UntitledTagGameRun.anim", true),
        jump = loadAnim(figure, "UntitledTagGameJump.anim", true),
        }
        animJump = baseAnims.jump

        actionAnims = {
        ["Tag 1"] = loadAnim(figure, "UntitledTagGameTag1.anim", false),
        ["Tag 2"] = loadAnim(figure, "UntitledTagGameTag2.anim", false),
        }
        ...
        ContextActions:BindAction("GM_Tag_1", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Tag 1") end
        end, true, Enum.KeyCode.Z)
        ContextActions:SetTitle("GM_Tag_1", "Tag 1")
        ContextActions:SetPosition("GM_Tag_1", UDim2.new(1, -80, 1, -100))

        ContextActions:BindAction("GM_Tag_2", function(_, inputState, _)
            if inputState == Enum.UserInputState.Begin then playAction("Tag 2") end
        end, true, Enum.KeyCode.X)
        ContextActions:SetTitle("GM_Tag_2", "Tag 2")
        ContextActions:SetPosition("GM_Tag_2", UDim2.new(1, -40, 1, -120))
    end
    ...
    m.Destroy = function(figure)
        ContextActions:UnbindAction("GM_Tag_1")
        ContextActions:UnbindAction("GM_Tag_2")
